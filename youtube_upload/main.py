...
def upload_youtube_video(youtube, options, video_path, total_videos, index):
    ...
    if options.notifySubscribers is "N":
    ...
    try:
        video_id = upload_video.upload(youtube, video_path, 
            request_body, progress_callback=progress.callback, notifySubscribers=notifySubscribers)
    finally:
        progress.finish()
    return video_id
...
def main(arguments):
    ...
    parser.add_option('', '--notifySubscribers', dest='notifySubscribers',
        type="string", default="True", metavar="string",
        help='Notify Subscribers? Y/N (default: Y)')
